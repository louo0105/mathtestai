...
### 6. 弱點抓取核心準則 (Weakness Capture Logic)
- **來源依據**：處理 ODS 學生評量檔案時，必須**嚴格且唯一**參考各知識點區塊的「**答對率**」欄位。
- **欄位定位**：在標準學力檢測 ODS 格式中，「答對率」通常位在該知識點代碼欄位向右偏移 **第 8 格**（即該節點數據區塊的最後一欄）。
- **判定門檻**：只要該欄位的數值 **小於 100**，即判定該生在此節點為「弱點」。
- **禁忌**：絕對不可參考「原始分數」、「答對題數」或「通過」等其他欄位進行初判，以確保所有未滿分的節點都能被精確補強。
